...
scenes/world/InnerNeraxShops.tscn    - the inner market's shopping street: 4 shopfronts
scenes/world/ButcherShop.tscn        - The Fatted Cleaver: combat_usable=false meat cuts
scenes/world/AlchemistShop.tscn      - The Gilded Retort: potions + the one-shot All-Stat Elixir
...
Market also has a third exit (east, "Ward Stairs") up into `NeraxUpperWard.tscn`, which itself continues north into `InnerWallSouth.tscn` and, through its guarded gate, `InnerNerax.tscn` - a fountain plaza that itself branches east into `InnerNeraxShops.tscn` and its four shop interiors (`ButcherShop.tscn`, `AlchemistShop.tscn`, `ClothingShop.tscn`, `RelicShop.tscn`). See "The Upper Ward, a beggar's gift, and the inner wall" below.
...
**The fountain is a real four-way hub, not just scenery.** South is the gate back to `InnerWallSouth`. North is a marching column of soldiers - a plain repeatable `MESSAGE` ("guards ... called to the royal castle"), not a real `EXIT`; it reads as an ongoing procession blocking the road rather than a one-shot flag, and deliberately doesn't lead anywhere yet (the road to the castle itself is future content). West is a similarly-blocked residential lane, reserved the same way. East is the only direction that actually goes somewhere new: `scenes/world/InnerNeraxShops.tscn`, a small shopping street.

**The shopping street has four shopfronts**, each the `GuildHallDoor`-style facade-plus-`EXIT`-door pattern Plaza already uses for the Adventure Centrum (a decorative building facade, no collision, with a circular-shape `EXIT` interactable in front of it) rather than a wall gap - leading to four distinct, small interior scenes. Every shop has 1-4 ambient flavor NPCs (plain `MESSAGE`s with no story relevance - regulars, apprentices, browsers) alongside its actual vendor stalls, so each interior feels inhabited rather than just a shelf of `TRADE` nodes:
...
- **The Gilded Retort** (Alchemist Shop) sells the usual Minor Healing/Mana Potions at an inner-market markup (10 gold each, repeatable, same as any vendor), plus the **All-Stat Elixir** - permanently +1 to every core stat, 150 gold, and only ever purchasable once. That "only once" is a second new generic mechanic: `Interactable.purchase_flag_id` (distinct from `story_flag_id`, which fires the instant an interactable is *triggered* - wrong for gating a purchase, since opening the buy prompt and declining or being short on gold would wrongly burn it). `ExplorationArea._handle_trade()` now checks the flag *before* showing the buy prompt, and only sets it in `_on_trade_confirmed()` after the gold is actually spent - so it's genuinely gated on a successful sale, not just an interaction. Consuming the elixir (Inventory screen only - it's `combat_usable = false` too) calls the new `RunManager.grant_all_stats_bonus(amount)`, which bumps every entry in `run.allocated_stats` directly, bypassing the normal unspent-points gate `allocate_stat_point()` enforces - appropriate for a rare one-off reward rather than the regular level-up flow.
